...
class RandpermOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddOutput("Out", "(Tensor). The output tensor of randperm op.");
    ...
    AddAttr<bool>("force_cpu",
                  "(bool, optional), Force the output variable to be "
                  "saved in cpu memory. Otherwise, save the output variable "
                  "to the running device. Default:false.")
        .SetDefault(false);
    AddAttr<int>("seed",
                 "(int, optional), Random seed used for permute samples. "
...
For example, if n = 4, the output may be tenor([3, 1, 0, 2]). 
)DOC");
  }
};
